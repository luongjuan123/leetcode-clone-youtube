...
import { FaPalette, FaUserCog, FaSave, FaUser, FaCamera, FaCheckCircle, FaShieldAlt } from "react-icons/fa";
...
export default function SettingsPage() {
	...
	return (
		<main className='bg-dark-layer-2 min-h-screen pb-16'>
			<Topbar />
			<div className='max-w-[1000px] mx-auto w-full mt-8 px-4'>
				...
				<div className='grid grid-cols-1 md:grid-cols-4 gap-8'>
					{/* Sidebar Navigation */}
					<div className='md:col-span-1 flex md:flex-col gap-2 border-b md:border-b-0 border-border-subtle pb-4 md:pb-0 overflow-x-auto md:overflow-visible shrink-0'>
						<button
							onClick={() => setActiveTab("profile")}
							className={`flex items-center gap-3 px-4 py-3 rounded-xl text-xs sm:text-sm font-bold transition-all whitespace-nowrap w-full text-left ${
								activeTab === "profile"
									? "bg-brand-orange/10 border border-brand-orange/30 text-brand-orange"
									: "text-dark-gray-8 hover:bg-dark-fill-3 border border-transparent hover:text-white"
							}`}
						>
							<FaUserCog size={16} />
					...
					{/* Main Content Area */}
					<div className='md:col-span-3 space-y-8 min-w-0'>
						{activeTab === "profile" && (
								...
								{user ? (
									<form onSubmit={handleSave} className='rounded-2xl p-6 space-y-6' style={{ background: "var(--bg-dark-layer-1)", border: "1px solid var(--border-subtle)" }}>
										...
										<div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
											...
											<div className='col-span-1 md:col-span-2' style={{ color: "var(--text-secondary)" }}>
												<label htmlFor='bio' className='text-sm font-semibold block mb-2'>
													Short Bio
												</label>
												<textarea
													value={profile.bio}
