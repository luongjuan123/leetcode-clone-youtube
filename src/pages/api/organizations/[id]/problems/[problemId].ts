...
async function handler(req: AuthenticatedRequest, res: NextApiResponse) {
	...
	const targetProblemId = problemId as string;
	...
	try {
		...
		// Security Check
		const { allowed } = await checkOrgPermission(org.id, uid, "organization.deleteProblem");
		if (!allowed) {
			return res.status(403).json({ success: false, error: "Forbidden: Insufficient Permissions" });
		}

		const linkDocId = `${org.id}_${targetProblemId}`;
		const linkRef = db.collection("organizationProblems").doc(linkDocId);
		const linkSnap = await linkRef.get();
		if (!linkSnap.exists) {
			return res.status(404).json({ success: false, error: "Not Found: Problem is not linked to this organization" });
		}

		const orgRef = db.collection("organizations").doc(org.id);

		// Transactional delete link: decrement problemCount
		await db.runTransaction(async (transaction) => {
			const orgSnap = await transaction.get(orgRef);
			if (!orgSnap.exists) throw new Error("Organization not found");
			const currentCount = orgSnap.data()?.problemCount || 1;
		...
		console.error("DELETE /api/organizations/:id/problems/:problemId error:", error);
